...
def full_prompt(record):
    ...
    return EXAMPLE_BLOCK + build_actual_prompt(record["case"], days)
...
def main():
    ...
    with open("evaluation/newfteval_newft.txt", "w", encoding="utf-8") as f:
    ...
    with open(test_json, "r", encoding="utf-8") as f:
        test_data = json.load(f)
    cnrs = [item['CNR'] for item in test_data]
    bail_types = [extract_bail_type(item['case']) for item in test_data]
    ...
    for i in range(len(prompts)):
        examples.append({
            ...
            'bail_type': bail_types[i],  # Extracted bail type
